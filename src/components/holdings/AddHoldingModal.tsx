...
const schema = z.object({
  ...
  name: z.string().min(1),
  amount: z.number().int().gt(0, "Amount must be a positive integer"),
});
...
export default function AddHoldingModal({
  ...
  useEffect(() => {
    const t = window.setTimeout(() => setDebouncedQuery(search.trim()), 300);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md bg-white rounded shadow-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium">Add New Holding</h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            <div className="text-sm text-gray-700 mb-1">Selected:</div>
